  ...
  try {
    ...
    if (opts.autoSize) {
      ...
      if (!isValidChunkSize(opts.size)) {
        logger.error(
          `Invalid chunk size: ${opts.size}. Must be one of: 1, 2, 3, or 4 (MB)`,
        );
        process.exit(1);
      }
      chunkSize = opts.size;
    ...
    for (const chunk of result.chunks) {
      logger.info(
        `  [${chunk.index}/${result.chunksCreated}] ${chunk.path} (${chunk.sizeMB} MB, ${chunk.pages} pages)`,
      );
    }
    logger.info('');
    logger.info(
      `Upload these chunks to Kiro in order. Each is a valid, standalone PDF.`,
    );
  } catch (err) {
    const msg = err instanceof Error ? err.message : String(err);
