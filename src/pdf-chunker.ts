...
async function pageByteSize(source: PDFDocument, pageIndex: number): Promise<number> {
 ...
 */
export async function chunkPdf(options: ChunkOptions): Promise<ChunkResult> {
  ...
  for (let i = 0; i < pageCount; i++) {
    ...
    if (pageSize > targetBytes && current.length === 0) {
      ...
      logger.warn(
        `Page ${i + 1} is ${(pageSize / MB).toFixed(2)}MB, larger than the ${chunkSizeMB}MB target. It will be its own (oversized) chunk.`,
      );
      groups.push([i]);
      current = [];
      continue;
  ...
  for (let g = 0; g < groups.length; g++) {
    ...
    if (!dryRun) {
      ...
      logger.success(
        `Created chunk ${g + 1}/${numChunks}: ${chunkFileName} (${sizeMB.toFixed(2)} MB, ${indices.length} pages)`,
      );
    } else {
      logger.info(
        `[DRY RUN] Would create chunk ${g + 1}/${numChunks}: ${chunkFileName} (${sizeMB.toFixed(2)} MB, ${indices.length} pages)`,
      );
    }
 ...
 */
export function suggestChunkSize(pdfSizeBytes: number): 1 | 2 | 3 | 4 {
  ...
  if (pdfSizeMB <= 4) return 4;
  return 4;
}

/** Validate that a chunk size is one of the supported values. */
export function isValidChunkSize(size: unknown): size is 1 | 2 | 3 | 4 {
