 ...
 */
export function parseFrontMatter(raw: string): { frontMatter: SpecFrontMatter; content: string } {
  const parsed = matter(raw);
  return { frontMatter: (parsed.data ?? {}) as SpecFrontMatter, content: parsed.content };
}
 ...
 */
export function deriveSpecId(slug: string, frontMatter: SpecFrontMatter): string {
...
export function requirementId(specId: string, number: number): string {
  return `${specId}:R${number}`;
}

/** Build a stable task id within a spec. */
export function taskId(specId: string, number: string): string {
  return `${specId}:T${number}`;
}

/** Build a stable diagram id within a spec (figure numbers are assigned separately). */
export function diagramId(specId: string, index: number): string {
...
const GENERIC_H1 =
  /^(requirements|design|implementation plan|tasks|technical design)( document)?$/i;
 ...
 */
export function resolveTitle(slug: string, frontMatter: SpecFrontMatter, h1?: string): string {
...
export function resolveVersion(frontMatter: SpecFrontMatter): string {
  if (frontMatter.version === undefined || frontMatter.version === null) return '—';
  return String(frontMatter.version);
}

export function resolveStatus(frontMatter: SpecFrontMatter): string {
