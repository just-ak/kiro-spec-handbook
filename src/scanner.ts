...
function classify(fileName: string): SpecDocument['kind'] {
...
function hashSpec(entries: Array<{ relPath: string; bytes: Buffer }>): string {
 ...
 */
export async function scanSpecs(config: HandbookConfig): Promise<Spec[]> {
  const specsRoot = resolveFromRepo(config, config.source.specs);
  const toRel = (abs: string): string => toPosix(relative(config.repoRoot, abs));
  ...
  for (const dir of dirs) {
    ...
    if (mdPaths.length === 0 && svgPaths.length === 0) continue;

    const hashInputs: Array<{ relPath: string; bytes: Buffer }> = [];
    ...
    const references: ReferenceFileRef[] = refPaths.map((absPath) => ({
      specId,
      absPath,
      relPath: toRel(absPath),
