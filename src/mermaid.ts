...
export function replaceMermaidBlocks(
  content: string,
  replacer: (block: MermaidBlock) => string,
): string {
  let i = 0;
  MERMAID_FENCE.lastIndex = 0;
  return content.replace(
    MERMAID_FENCE,
    (_full, lead: string, _fence: string, code: string) => {
...
export function findMmdc(repoRoot: string): string | null {
...
export async function renderMermaidToSvg(
  ...
  try {
    await execa(
      mmdc,
      ['-i', mmdPath, '-o', svgDest, '-b', 'white', '-p', puppeteerCfg, '-c', mermaidCfg],
      { env, cwd: workDir },
    );
