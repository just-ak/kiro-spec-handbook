...
export function parseRequirements(specId, content) {
    const out = [];
    const seen = new Set();
 ...
 */
export function parseTasks(specId, content) {
    const lines = content.split(/\r?\n/);
    const out = [];
    for (let i = 0; i < lines.length; i++) {
        ...
        const requirementNumbers = [];
        for (let j = i + 1; j < lines.length; j++) {
            if (TASK_LINE.test(lines[j]))
                break;
            ...
            if (rm) {
                for (const part of rm[1].split(',')) {
                    ...
                    if (cleaned)
                        requirementNumbers.push(cleaned);
                }
            }
        }
        out.push({
