...
import { diagramIndex, requirementsIndex, specIndex, tasksIndex, traceabilityMatrix, } from './indexer.js';
import { buildLock, computeDelta, readLock, serialiseLock } from './lock.js';
import { logger } from './logger.js';
import { render } from './renderer.js';
import { scanSpecs } from './scanner.js';
import { appendices, architectureDecisions, architectureOverview, documentInfo, steeringSection, } from './sections.js';
...
async function resolveBuildDate(config) {
    if (config.build_date && config.build_date.trim())
        return config.build_date.trim();
...
async function assembleParts(config, handbook) {
    return {
...
program
    ...
    logger.success(`Lock updated: +${delta.added.length} new, ~${delta.changed.length} changed, -${delta.removed.length} removed.`);
    ...
    try {
        ...
        if (autoSize) {
            ...
            if (!isValidChunkSize(parsedSize)) {
                logger.error(`Invalid chunk size: ${sizeStr}. Must be one of: 1, 2, 3, or 4 (MB)`);
                process.exitCode = 1;
                return;
            }
        ...
        for (const chunk of result.chunks) {
            logger.info(`  [${chunk.index}] ${chunk.path} (${chunk.sizeMB} MB, ${chunk.pages} pages)`);
        }
    }
    catch (err) {
