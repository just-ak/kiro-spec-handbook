...
import { anchors, buildReferenceIndex, injectRequirementAnchors, linkifyRequirementRefs, linkifySpecPaths, } from './references.js';
import { captionFromFileName } from './markdown.js';
import { extractMermaidBlocks, findMmdc, renderMermaidToSvg, replaceMermaidBlocks, } from './mermaid.js';
...
export function shiftHeadings(content, by) {
    if (by <= 0)
        return content;
    return content.replace(/^(#{1,6})(\s+)/gm, (_m, hashes, space) => {
 ...
 */
async function resolveSteeringIncludes(steeringMarkdown, config, outputDir, converter) {
    if (!steeringMarkdown.includes('#[[file:'))
        return steeringMarkdown;
    ...
    for (const line of lines) {
        ...
        out.push(line.replace(FILE_INCLUDE, (_m, p) => `\`${p.trim()}\``));
    }
    return out.join('\n');
}
/** Replace mermaid fenced blocks with rendered figure images (or keep as code). */
function embedMermaid(content, spec, docKind, mermaidImages) {
...
function embedDocument(doc, spec, refIndex, mermaidImages) {
    ...
    if (doc.kind === 'requirements')
        content = injectRequirementAnchors(content, spec.slug);
    if (doc.kind === 'tasks')
        content = linkifyRequirementRefs(content, spec.slug);
    content = stripFirstH1(content);
    return shiftHeadings(content, 2);
}
/** Detect an available SVG→PDF converter for LaTeX embedding. */
async function detectSvgConverter() {
    for (const tool of ['rsvg-convert', 'inkscape', 'cairosvg']) {
        try {
...
async function convertSvg(tool, src, dest) {
    try {
 ...
 */
async function prepareDiagrams(handbook, outputDir, converter) {
 ...
 */
async function prepareMermaid(handbook, config, outputDir, converter) {
    const map = new Map();
    const EMBEDDED_KINDS = ['requirements', 'design', 'tasks'];
    const total = handbook.specs.reduce((n, s) => n +
        s.documents.reduce((m, d) => m + (EMBEDDED_KINDS.includes(d.kind) ? extractMermaidBlocks(d.content).length : 0), 0), 0);
    if (total === 0)
        return map;
    const mmdc = findMmdc(config.repoRoot);
    if (!mmdc) {
        logger.warn('mermaid-cli (mmdc) not found. Install it with `npm i -D -w @ledgiventa/handbook @mermaid-js/mermaid-cli`. Mermaid blocks will render as code.');
        return map;
    }
    ...
    for (const spec of handbook.specs) {
        for (const doc of spec.documents) {
            if (!EMBEDDED_KINDS.includes(doc.kind))
                continue;
...
function diagramsSection(spec, images) {
...
function renderSpec(spec, refIndex, images, mermaidImages) {
    const out = [];
...
function metadataBlock(handbook, config) {
 ...
 */
export function assembleMarkdown(handbook, config, parts, images, mermaidImages = new Map()) {
    ...
    if (config.indexes.specs)
        sections.push(parts.specIndex);
    if (config.indexes.requirements)
        sections.push(parts.requirementsIndex);
    if (config.indexes.tasks)
        sections.push(parts.tasksIndex);
    if (config.indexes.diagrams)
        sections.push(parts.diagramIndex);
    ...
    if (config.traceability.enabled)
        sections.push(parts.traceability);
    if (config.indexes.changes)
        sections.push(parts.gitHistory);
 ...
 */
export async function render(handbook, config, parts) {
    const outputDir = resolveFromRepo(config, config.output.directory);
    mkdirSync(outputDir, { recursive: true });
    const converter = await detectSvgConverter();
    if (!converter) {
        logger.warn('No SVG→PDF converter found (rsvg-convert / inkscape / cairosvg). Diagrams will be listed but not embedded in the PDF.');
    }
    ...
    if (!pandocAvailable) {
        logger.warn('pandoc not found on PATH — skipping PDF generation. Install pandoc + a LaTeX engine (xelatex) to produce the PDF. The assembled markdown above is ready to render.');
...
async function runPandoc(config, markdownPath, pdfPath, cwd) {
